...
const allowedOrigins = ['https://teams.microsoft.com'];
...
emoticonRouter.get('/emoticon/:emoticon', async (req, res) => {
    ...
    console.log(emoticonName);
    let imageData: Buffer;
    fs.readFile(`${__dirname}/${emoticonName}.gif`)
    .then(file => {
        console.log(file);
        ...
        .then(files => {
            files.forEach(file => {
                if (file.endsWith(ext)) {
                    console.log(file);
                    let fileLessExt = file.substring(0, file.lastIndexOf(ext));
                    emojis.push({ "name": fileLessExt });
                }

            });
            console.log(emojis);
            res.send(emojis.map(row => row.name));
            return;
        }
        )
        .catch(err => {
